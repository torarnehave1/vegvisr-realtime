 ...
 *   somehow renders this gets 403 and nothing shows.
 * - Impersonating swaps localStorage['user'] for the target's auth payload and
 *   stashes the owner's real login in localStorage['originalUser'] so it can be
 *   restored with one click. The owner's own token is never sent to the client
 *   of the impersonated session beyond what they already hold.
 */
  ...
  if (impersonating) {
    return (
      <div className="flex items-center justify-between gap-3 px-4 py-2 bg-amber-600 text-white text-sm">
        ...
        <button
          className="px-3 py-1 bg-amber-800 hover:bg-amber-900 rounded text-xs font-medium whitespace-nowrap"
          onClick={stop}
        >
