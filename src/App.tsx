...
function RealtimeMeeting() {
  ...
  const [syncingStream, setSyncingStream] = useState(false);
  ...
  const [metadataDraft, setMetadataDraft] = useState<{ title: string; labels: string; thumbnailUrl: string; category: string; metaArea: string; description: string }>({ title: '', labels: '', thumbnailUrl: '', category: '', metaArea: '', description: '' });
  // Key of the recording whose description is currently being generated by AI (Claude Haiku).
  const [generatingDescKey, setGeneratingDescKey] = useState<string | null>(null);
    ...
    try {
      const r = await fetch('https://api.vegvisr.org/realtime/recordings/metadata', {
        ...
        body: JSON.stringify({
          ...
          metaArea: metadataDraft.metaArea,
  ...
  // Ask Claude (Haiku, via the anthropic worker) to suggest a description + category +
  // metaArea from the recording's transcript. Requires a transcript to already exist —
  // the user produces it with the Transcribe button (same client-side pipeline).
  const generateDescriptionFromTranscript = async (key: string) => {
    const stored = readStoredUser();
    if (!stored?.emailVerificationToken) return;
    const transcript = (transcripts[key] || '').trim();
    if (!transcript || /^(Downloaded|Transcribing|Downloading)/.test(transcript)) {
      alert('Transcribe this recording first — then Generate can summarise it.');
      return;
    }
    setGeneratingDescKey(key);
    try {
      const r = await fetch('https://api.vegvisr.org/realtime/recordings/describe', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-API-Token': stored.emailVerificationToken,
      ...
      const data = await r.json();
      if (!r.ok || !data.success) throw new Error(data.error || `Describe failed with status ${r.status}`);
      setMetadataDraft(prev => ({
        ...prev,
        description: data.description || prev.description,
        // Only fill category/metaArea if the AI returned them and the field is empty —
        // never overwrite what the user already typed.
        category: prev.category || data.category || '',
        metaArea: prev.metaArea || data.metaArea || '',
      }));
    } catch (err: any) {
  ...
  // Copy Cloudflare Stream recordings → R2 (stream-recordings/ prefix). Mirrors
  // syncRecordingsToR2 but hits /stream-sync; the jobs land in the same
  // r2_sync_jobs table so the same sync-status polling + panel apply.
  const syncStreamRecordingsToR2 = async () => {
    const stored = readStoredUser();
    if (!stored?.emailVerificationToken) return;
    const targetEmail = activeAccount && activeAccount !== stored.email ? activeAccount : null;
    setSyncingStream(true);
    setSyncJobs([]);
    try {
      const r = await fetch('https://api.vegvisr.org/realtime/recordings/stream-sync', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-API-Token': stored.emailVerificationToken,
        },
        body: JSON.stringify(targetEmail ? { ownerEmail: targetEmail } : {}),
      });
      const data = await r.json();
      if (!data.success) {
        alert('Stream sync failed to start: ' + (data.error || 'Unknown error'));
        return;
      }
      ...
      if (queuedJobs.length === 0) {
        alert(
          `No new stream recordings queued. ${data.skipped || 0} already in R2` +
          (processing ? `, ${processing} still transcoding — press again shortly.` : '.')
        );
        await fetchRecordings();
        return;
      }
      ...
      const failed = (finalSd.jobs || []).filter((j: any) => j.status === 'failed').length;
      alert(
        `Stream sync complete: ${done} uploaded, ${failed} failed.` +
        (processing ? ` (${processing} were still transcoding — press again shortly.)` : '')
      );
      await fetchRecordings();
    } catch (err: any) {
      alert('Stream sync error: ' + err.message);
    } finally {
      setSyncingStream(false);
    }
  };
          ...
          <div className="max-w-4xl mx-auto">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-2">
                ...
                <button
                  className="px-3 py-2 bg-purple-700 hover:bg-purple-600 rounded text-white text-xs disabled:opacity-40"
                  disabled={syncingStream}
                  onClick={syncStreamRecordingsToR2}
                  title="Copy Cloudflare Stream (live broadcast) recordings into R2"
                >
                  {syncingStream ? 'Syncing…' : '📡→💾 Sync stream recordings'}
                </button>
                <button
                        ...
                        <div className="mt-3 border border-slate-600 rounded-lg p-3 bg-slate-900/70 space-y-2">
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => setMetadataDraft(prev => ({ ...prev, category: e.target.value }))}
                              placeholder="Onboarding, Interview, Lecture…"
                            />
                          </div>
                          <div>
                            <label className="block text-slate-400 text-xs mb-1">Meta area (#tags)</label>
                            <input
                              type="text"
                              className="w-full bg-slate-900 border border-slate-600 rounded px-2 py-1.5 text-white text-xs focus:outline-none focus:border-sky-500"
                              value={metadataDraft.metaArea}
                              onChange={(e) => setMetadataDraft(prev => ({ ...prev, metaArea: e.target.value }))}
                              placeholder="#Onboarding #HR #Product"
                            />
                          </div>
                          <div>
                            <div className="flex items-center justify-between mb-1">
                              <label className="block text-slate-400 text-xs">Description</label>
                              <button
                                type="button"
                                className="px-2 py-0.5 bg-purple-600 hover:bg-purple-500 rounded text-white text-[11px] disabled:opacity-40"
                                onClick={() => generateDescriptionFromTranscript(rec.key)}
                                disabled={generatingDescKey === rec.key}
                                title={transcripts[rec.key] ? 'Summarise the transcript with AI' : 'Transcribe the recording first'}
                              >
                                {generatingDescKey === rec.key ? 'Generating…' : '✨ Generate from transcript'}
                              </button>
                            </div>
                            <textarea
                              rows={3}
                              ...
                              onChange={(e) => setMetadataDraft(prev => ({ ...prev, description: e.target.value }))}
